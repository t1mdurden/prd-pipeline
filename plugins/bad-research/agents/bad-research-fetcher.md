...
/private/var/folders/jm/x74dlk1s4_q_982rmn3dxcx80000gn/T/tmp.vpH9Y31rLu/venv/bin/bad fetch --help >/dev/null 2>&1 && echo fetch_ok || echo fetch_missing
```
...
```bash
PYTHONIOENCODING=utf-8 /private/var/folders/jm/x74dlk1s4_q_982rmn3dxcx80000gn/T/tmp.vpH9Y31rLu/venv/bin/bad fetch "<url>" --tag <topic> -j
```
...
```bash
PYTHONIOENCODING=utf-8 /private/var/folders/jm/x74dlk1s4_q_982rmn3dxcx80000gn/T/tmp.vpH9Y31rLu/venv/bin/bad fetch "<url>" \
  --tag <topic> \
...
1. Check if it's already fetched (dedup — a prior fetch records the URL in the note):
   `PYTHONIOENCODING=utf-8 /private/var/folders/jm/x74dlk1s4_q_982rmn3dxcx80000gn/T/tmp.vpH9Y31rLu/venv/bin/bad search "<url>" -j`

2. If not already fetched, fetch it:
   `PYTHONIOENCODING=utf-8 /private/var/folders/jm/x74dlk1s4_q_982rmn3dxcx80000gn/T/tmp.vpH9Y31rLu/venv/bin/bad fetch "<url>" --tag <topic> -j`

3. After fetching, read the note content:
   `PYTHONIOENCODING=utf-8 /private/var/folders/jm/x74dlk1s4_q_982rmn3dxcx80000gn/T/tmp.vpH9Y31rLu/venv/bin/bad note show <note-id> -j`
...
4. **Quality check** — read the content and decide:
   ...
   available, `PYTHONIOENCODING=utf-8 /private/var/folders/jm/x74dlk1s4_q_982rmn3dxcx80000gn/T/tmp.vpH9Y31rLu/venv/bin/bad note update <note-id> --status deprecated -j`;
...
5. If the content is good, write a real summary and add tags. If `note update`
   is available:
   `PYTHONIOENCODING=utf-8 /private/var/folders/jm/x74dlk1s4_q_982rmn3dxcx80000gn/T/tmp.vpH9Y31rLu/venv/bin/bad note update <note-id> --summary "<specific summary>" -j`
   `PYTHONIOENCODING=utf-8 /private/var/folders/jm/x74dlk1s4_q_982rmn3dxcx80000gn/T/tmp.vpH9Y31rLu/venv/bin/bad note update <note-id> --add-tag <specific-tag> -j`
...
2. **Find and fetch the primary sources.** For each priority lead:
   - If you have a direct URL from the citation, fetch it with the
     ...
     ```
     PYTHONIOENCODING=utf-8 /private/var/folders/jm/x74dlk1s4_q_982rmn3dxcx80000gn/T/tmp.vpH9Y31rLu/venv/bin/bad search "<url>" -j   # dedup: already fetched?
     PYTHONIOENCODING=utf-8 /private/var/folders/jm/x74dlk1s4_q_982rmn3dxcx80000gn/T/tmp.vpH9Y31rLu/venv/bin/bad fetch "<url>" --tag <topic> --suggested-by <note-id-that-cited-it> --suggested-by-reason "cited as primary source" -j
     ```
   ...
   - Once you have the URL, fetch it with `/private/var/folders/jm/x74dlk1s4_q_982rmn3dxcx80000gn/T/tmp.vpH9Y31rLu/venv/bin/bad fetch` as above.
...
3. **Process each discovered source** with the same full procedure as
   Phase 1: read the note content with `/private/var/folders/jm/x74dlk1s4_q_982rmn3dxcx80000gn/T/tmp.vpH9Y31rLu/venv/bin/bad note show <id> -j`,
   quality check, write summary with `/private/var/folders/jm/x74dlk1s4_q_982rmn3dxcx80000gn/T/tmp.vpH9Y31rLu/venv/bin/bad note update`, add tags,
