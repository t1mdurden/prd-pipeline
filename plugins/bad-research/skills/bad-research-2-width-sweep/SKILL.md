...
- **light tier:** Skip directly to step 10 — invoke `Skill(skill: "bad-research:bad-research-10-triple-draft")` (light tier writes a single draft, not the ensemble)
- **full tier:** Invoke `Skill(skill: "bad-research:bad-research-4-loci-analysis")` (its Step 4.0 preamble builds the contradiction graph before loci analysis)
