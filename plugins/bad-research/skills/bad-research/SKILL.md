...
```
Skill(skill: "bad-research:bad-research-N-stepname")
```
...
- 0.5 → `Skill(skill: "bad-research:bad-research-0.5-clarify")` (triage clarifier; runs first on every interactive run, skipped only on `--auto`/wrapped runs)
- 1.5 → `Skill(skill: "bad-research:bad-research-query-router")` (the route decision)
- 1.6 → `Skill(skill: "bad-research:bad-research-1.6-plan-gate")` (user-editable plan-gate; interactive + full-route-or-broad-survey only, skipped on non-interactive / `--auto` / wrapped / small bounded runs)
- fast → `Skill(skill: "bad-research:bad-research-fast")` (bounded-ReAct = a step-capped Reason+Act loop; replaces 2–14; its breadth branch spawns K parallel researchers for a wider mid-tier answer)
- 11.5 → `Skill(skill: "bad-research:bad-research-11.5-citation-verifier")` (backward grounding = binding each report claim back to its source note; full only)
- 12.5 → `Skill(skill: "bad-research:bad-research-12.5-grader")` (in-pipeline grader loop: judge→patch→re-judge ≤3; full only — slots between critics/gap-fetch and the patcher's final convergence)
- 14.5 → `Skill(skill: "bad-research:bad-research-fresh-review")` (one fresh-context pass; full only)
...
0. **Auto-init if missing.** Two checks for the first-run-after-global-install case:
   ...
   - **Step-skills check.** Nothing to install: every step skill ships bundled inside this plugin as `bad-research:bad-research-N-...`, so the `Skill(skill: "bad-research:bad-research-N-...")` calls in later steps resolve from the session's registry. Do NOT run `bad install --steps-only` — that is the standalone install path, and here it would spray a second, diverging copy of all 20 step-skill directories into the working repo that nothing ever prunes.

   **CLI path.** Use `bad` for every command below. If a bare `bad …` call fails with "command not found" / exit 127 (common on a `uv tool` / global install where the binary is installed but not on PATH), use the absolute CLI path documented at the top of this project's CLAUDE.md (`**CLI path: `…`**`) for every command in this skill — the installer resolves and templates it there. Only if no such path exists, fall back to `${CLAUDE_PLUGIN_ROOT}/bin/bad` — this plugin ships the engine source and bootstraps it into its own data dir on first call, so the binary builds itself; if that reports no usable interpreter, the machine needs Python 3.11–3.13 plus `uv` or `pipx`. `bad init` no-ops cheaply when the vault already exists — safe to run unconditionally.
...
7. **Invoke the clarifier (step 0.5)** UNLESS this is an `--auto` / wrapped run
   ...
   to step 1:
   `Skill(skill: "bad-research:bad-research-0.5-clarify")`. The clarifier is triage-tier,
   default-proceed, ≤3 questions; it writes `research/clarify.json`.

8. **Invoke step 1 (decompose):** `Skill(skill: "bad-research:bad-research-1-decompose")`.

9. **Invoke step 1.5 (the query router):** `Skill(skill: "bad-research:bad-research-query-router")`.
...
10. **Invoke step 1.6 (the plan-gate)** for the `full` route:
    `Skill(skill: "bad-research:bad-research-1.6-plan-gate")`. It self-decides via
...
`Skill(skill: "bad-research:bad-research-fast")` then run the slim citation-grounding pass and
...
4. **Re-invoke this entry skill** if you've lost track entirely: `Skill(skill: "bad-research:bad-research")`. It loads fresh.
...
```
Skill(skill: "bad-research:bad-research-1-decompose")
```
