...
while revisions < MAX_GRADER_REVISIONS:   # 3
    ...
    Skill(skill: "bad-research:bad-research-14-patcher")   # the patcher reads critic-findings-grader.json too
    revisions += 1
...
3.5. **Accumulate the `grader_history` failure ledger (round N ≥ 2).** Before
   ...
   **composes with the C-4 round-1 aggregate path**: the `findings_applied` count
   ...
   "
   ```
4. Re-judge after patching: re-run the patcher (`Skill(skill: "bad-research:bad-research-14-patcher")`).
...
```
Skill(skill: "bad-research:bad-research-fresh-review")
```
